...
def _parse_proto(prototxt_fname):
    ...
    for i, layer in enumerate(layers):
        ...
        if layer.type == 'BatchNorm':
            ...
            need_flatten[name] = need_flatten[mapping[layer.bottom[0]]]
        if layer.type == 'Scale':
            if layers[i-1].type == 'BatchNorm':
                need_flatten[name] = need_flatten[mapping[layer.bottom[0]]]
                skip_layer = True
                prev_name = re.sub('[-/]', '_', layers[i-1].name)
            else:
                type_string = 'single_scale'
                need_flatten[name] = False
        if layer.type == 'PReLU':
        ...
        if layer.type == 'Reshape':
            ...
            param_string = "shape=(%s, -2)" % (','.join(map(str, param.shape.dim)),)
        ...
        elif type_string == 'single_scale':
            b1_name = layer.bottom[0].replace('/', '_')
            b2_name = layer.bottom[1]
            symbol_string += '%s = mx.sym.broadcast_mul(%s, %s)\n' % (name, b1_name, b2_name)
